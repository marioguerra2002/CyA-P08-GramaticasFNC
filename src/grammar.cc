...
std::ostream &operator<<(std::ostream &os, const Grammar &grammar) {
  ...
  if (!new_grammar.CheckGrammar()) {
    os << "La gramatica no es correcta" << std::endl;
    os << "Asegurate de que no tenga producciones vacias y" << std::endl;
    os << "que no tenga producciones unitarias" << std::endl;
  } else {

    os << grammar.terminals_.size() << std::endl;
    for (auto &i : grammar.terminals_) {
      os << i.GetSymbol() << std::endl;
    }
    os << grammar.nonterminals_.size() << std::endl;
    for (auto &i : grammar.nonterminals_) {
      if (i.GetSymbol() == "S") {
        os << i.GetSymbol() << std::endl;
      }
    }
    ...
    productions = grammar.productions_;
    
    /// recorro el vector de producciones de la regla S
    for (unsigned long int i{0}; i < productions[Symbol("S")].size(); ++i) {  
      os << "S -> ";
      /// recorro el vector de símbolos de cada producción
      for (unsigned long int j{0}; j < productions[Symbol("S")][i].size(); ++j) {
        os << productions[Symbol("S")][i][j].GetSymbol();
      }
      os << std::endl;
    }
    for (auto& i : productions) {
      if (i.first.GetSymbol() != "S") {
        for (unsigned long int j{0}; j < i.second.size(); ++j) {
          os << i.first.GetSymbol() << " -> ";
          for (unsigned long int k{0}; k < i.second[j].size(); ++k) {
            os << i.second[j][k].GetSymbol();
          }
          os << std::endl;
        }
      }
    ...
    return os;
    for (auto& i : grammar.productions_) {
      os << i.first.GetSymbol() << " -> ";
      for (unsigned long int j{0}; j < i.second.size(); ++j) {
        for (unsigned long int k{0}; k < i.second[j].size(); ++k) {
          os << i.second[j][k].GetSymbol();
